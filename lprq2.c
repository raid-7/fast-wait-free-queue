...
#define RING_SIZE LPRQ2_RING_SIZE

static inline int is_empty(uint64_t v) __attribute__ ((pure));

static inline int is_bottom(uint64_t v) __attribute__ ((pure));

static inline uint64_t get_bottom(handle_t* th) __attribute__ ((pure));
...
inline int is_bottom(uint64_t v) {
    return (v & ((1ull << 63))) != 0;
}

inline uint64_t get_bottom(handle_t* th) {
    return ((uint64_t) th) | (1ull << 63);
}
...
static void lprq_put(queue_t* q, handle_t* handle, uint64_t arg) {
    ...
    while (1) {
        ...
        if ((is_empty(val) || is_bottom(val)) && node_index(idx) <= t && (!node_unsafe(idx) || rq->head < t)) {
            uint64_t bottom = get_bottom(handle);
            if (CAS(&cell->val, &val, bottom)) {
                if (CAS(&cell->idx, &idx, t + RING_SIZE)) {
                    if (CAS(&cell->val, &bottom, arg)) {
                        return;
                    }
                } else {
                    CAS(&cell->val, &bottom, (uint64_t) -1);
                }
            }
        }
...
static uint64_t lprq_get(queue_t* q, handle_t* handle) {
    while (1) {
        ...
        while (1) {
            ...
            if (!is_empty(val) && !is_bottom(val)) {
                if (idx == effective_h) {
                    SWAP(&cell->val, -1);
                    return val;
                } else {
                    if (CAS(&cell->idx, &cell_idx, set_unsafe(idx))) {
                        break;
                    }
                }
            } else {
                ...
                if (is_bottom(val) && !CAS(&cell->val, &val, (uint64_t)-1))
                    continue;
